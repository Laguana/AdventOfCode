...
func TestPrismSubtraction(t *testing.T) {
	a := prism{x: pair{0, 2}, y: pair{0, 2}, z: pair{0, 2}}

	asuba := a.subtract(a)
	if len(asuba) != 0 {
		t.Errorf("Expected empty result, got %v", asuba)
	}

	b := prism{x: pair{1, 1}, y: pair{1, 1}, z: pair{1, 1}}

	if !a.collides(b) {
		t.Errorf("b should collide with b!")
	}

	asubb := a.subtract(b)
	if len(asubb) != 6 {
		t.Errorf("Expected 6 prisms, got %v", asubb)
	}
}
