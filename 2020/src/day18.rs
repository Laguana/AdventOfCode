...
fn part1_sample_works() {
    ...
    assert_eq!(evaluate(&parse_expr("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))")),12240);
    assert_eq!(evaluate(&parse_expr("((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2")),13632);
...
fn parse_expr_paren(input: &[Token], i: usize) -> (usize, Expression) {
    ...
    let (j,first) = match input[i] {
        Token::Int(v) => {
            (i+1, Expression::Literal(v))
        },
        Token::Char('(') => {
            parse_expr_paren(input, i+1)
        },
...
fn parse_input(input: &str) -> Vec<Expression> {
    input.trim().split("\n").map(|l| parse_expr(l.trim())).collect()
}
